...
public class Sql99DslProvider implements DslProvider {

    public static final String         NAME        = Sql99SqlEngineSpi.NAME;
    public static final DslProvider    INSTANCE    = new Sql99DslProvider();
    private final AntlrStatementParser TREE_PARSER = new Sql99StatementParser();
    ...
    public Lexer createLexer(CharStream charStream) {
        return new Sql99Lexer(new CaseInsensitiveStream(charStream));
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new Sql99Parser(new CommonTokenStream(lexer));
    }
