...
public class Sql92DslProvider implements DslProvider {

    public static final DslProvider    INSTANCE    = new Sql92DslProvider();
    private final AntlrStatementParser TREE_PARSER = new Sql92StatementParser();

    @Override
    public String[] getDslName() { return new String[] { Sql92SqlEngineSpi.NAME }; }

    @Override
    public Lexer createLexer(CharStream charStream) {
        return new Sql92Lexer(new CaseInsensitiveStream(charStream));
    }

    @Override
    public Parser createParser(Lexer lexer) {
        return new Sql92Parser(new CommonTokenStream(lexer));
    }
